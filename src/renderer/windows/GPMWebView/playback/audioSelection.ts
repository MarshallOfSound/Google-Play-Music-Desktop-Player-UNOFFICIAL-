...
Emitter.on('audiooutput:fetch', () => {
  navigator.mediaDevices.enumerateDevices()
    .then((devices) => {
      Emitter.fireAtAll('audiooutput:list', _.transform(devices, (final, device: GPMDP.AudioDevice) => {
        final.push({
...
export const setAudioDevice = (id, count = 0) =>
  new Promise<void>((resolve, reject) => {
    (<HTMLAudioElement & { setSinkId: Function }>document.querySelectorAll('audio')[0]).setSinkId(id)
      .then(() => { resolve(null); })
