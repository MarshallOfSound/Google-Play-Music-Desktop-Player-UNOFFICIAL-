...
const hide = (elementSelector: string, kill: boolean = false) => {
  const nodeList = document.querySelectorAll(elementSelector);
  _.forEach(nodeList, (node: HTMLElement) => {
    const element = node;
...
export const style = (elementSelector: string, styleObject: any) => {
  const nodeList = document.querySelectorAll(elementSelector);
  _.forEach(nodeList, (node: HTMLElement) => {
    const element = node;
...
export const cssRule = (styles: string) => {
...
// --- UI modifications ---
function _redirectButton(button: HTMLElement, URL: string, reverseURLChange: boolean = false) {
  if (button) {
...
function fixShopButton() {
  _redirectButton(<HTMLElement>document.querySelector('[data-type="shop"]'), 'https://play.google.com/store/music?feature=music_general');
}

function handleSubscribeButton() {
  _redirectButton(<HTMLElement>document.querySelector('[data-type="sub"]'), 'https://play.google.com/music/listen#/sulp', true);
}
...
function installSidebarButton(translationKey: string, type: string, icon: string, index: number, fn: (e?: Event) => boolean) {
...
function installNowPlayingButton(textKey: string, id: string, callback: (event: Event) => void) {
  ...
  button.addEventListener('click', (e) => {
    ...
    setTimeout(() => {
      (<HTMLElement>document.querySelector('.goog-menu.song-menu')).style.display = 'none';
    }, 150);
    callback(e);
  });
}
