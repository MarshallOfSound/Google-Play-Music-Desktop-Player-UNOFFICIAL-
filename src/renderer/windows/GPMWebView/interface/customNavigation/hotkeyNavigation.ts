...
(<any>window).wait(() => {
  ...
  const backBtn = <HTMLButtonElement>document.createElement('paper-icon-button');
  ...
  const attemptBack = () => {
    ...
    remote.getCurrentWebContents().executeJavaScript(testJs, false, (canGoBack: boolean) => {
  ...
  const attemptForward = () => {
    ...
    remote.getCurrentWebContents().executeJavaScript(testJs, false, (canGoForward: boolean) => {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((holdingKeyMap[91] || holdingKeyMap[93])
       && (<HTMLInputElement>document.activeElement).value === undefined) {
      if (e.which === 37) {
        attemptBack();
      } else if (e.which === 39) {
        attemptForward();
  ...
  window.addEventListener('keyup', (e) => {
    if ((e.which === 8 && (<HTMLInputElement>document.activeElement).value === undefined)
      || (e.which === 37 && e.altKey && (<HTMLInputElement>document.activeElement).value === undefined)) {
      attemptBack();
    } else if (e.which === 39 && e.altKey && (<HTMLInputElement>document.activeElement).value === undefined) {
      attemptForward();
    }
