...
export default function (playlistName) {
  return new Promise((resolve, reject) => {
    if (!playlistName) { resolve(null); return; }
    ...
    if (foundLink) {
      foundLink.click();
      waitFor(() => {
        const title = <HTMLElement>document.querySelector('.playlist-view .material-container-details .info .title'); // eslint-disable-line
        return title && title.innerText.toLowerCase().trim() === playlistName.toLowerCase();
      })
        .then(() => {
          (<HTMLButtonElement>document.querySelector('.playlist-view .material-container-details [data-id=play]')).click(); // eslint-disable-line
          resolve('');
        });
    } else {
