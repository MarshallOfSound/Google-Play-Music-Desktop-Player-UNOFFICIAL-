...
const receiverFn = (receivers) =>
  new (<any>window).Promise((resolve, reject) => {
    let dialog: GPMDP.PaperDialog = <GPMDP.PaperDialog>document.createElement('paper-dialog');
    trigID++;
    ...
    receivers.forEach((receiver) => {
      ...
      document.body.addEventListener('click', (e) => {
        ...
        if ((<any>e.target).getAttribute('data-reciever-id') === `${receiver.ipAddress}_${trigID}`) {
          dialog.close();
