...
let server: WebSocket.Server & GPMDP.API.WebSocketServer;
let oldTime = {};
let uniqID = uuid.v4();

let runas = (cmd, args, opts) => {};
...
const API_VERSION: string = JSON.parse(fs.readFileSync(path.resolve(`${__dirname}/../../../../package.json`), 'utf8')).apiVersion;
...
const enableAPI = () => {
  ...
  server = <WebSocket.Server & GPMDP.API.WebSocketServer>new WebSocketServer({
    ...
    server.broadcast = (channel: string, data: Object) => {
      server.clients.forEach((client: WebSocket & GPMDP.API.WebSocketClient) => {
    ...
    server.on('connection', (websocket) => {
      const ws = <WebSocket & GPMDP.API.WebSocketClient>websocket;

      ws.json = (obj: Object) => {
        ws.send(JSON.stringify(obj));
      };
      ws.channel = (channel: string, obj: Object) => {
        ws.json({
          channel,
          payload: obj,
        });
